...
$(document).ready(function () {
  ...
  // Game Menu
  function menu () {
    if (typeof backtomenu_timeout !== 'undefined') { clearTimeout(backtomenu_timeout) }
  ...
  $('#walls').change(function () {
    if (this.checked) { walls = true } else { walls = false }
  })

  $('#sounds').change(function () {
    if (this.checked) { sounds = true } else { sounds = false }
  })

  $('input[name=snake_weight]').change(function () {
    cell_size = 10
    if ($('input[name=snake_weight]:checked').val() == 5) { cell_size = 5 }
  })
  ...
  function endGame () {
    if (typeof game_loop !== 'undefined') {
      ...
      var screenshotJSON = JSON.stringify(snake_array)
  ...
  function trollolol () {
    ...
    ctx.clearRect(0, 0, canvas.width, canvas.height)
    $('#nothingsuspicious').html('<iframe width="413" height="310" src="https://www.youtube.com/embed/oavMtUWDBTM?autoplay=1" frameborder="0" allowfullscreen></iframe>')
  ...
  function draw () {
    ...
    if (keystroke_array[0] != null) {
      ...
      if (next_move == 'left' && direction != 'right') { direction = 'left' } else if (next_move == 'up' && direction != 'down') { direction = 'up' } else if (next_move == 'right' && direction != 'left') { direction = 'right' } else if (next_move == 'down' && direction != 'up') { direction = 'down' }
    }
    ...
    if (direction == 'right') { snake_head_x++ } else if (direction == 'left') { snake_head_x-- } else if (direction == 'down') { snake_head_y++ } else if (direction == 'up') { snake_head_y-- }

    // Allow the snake to travel through the walls if the user has opted to disable walls.
    if (!walls) {
      if (snake_head_x <= -1) { snake_head_x = Math.abs((w - Math.abs(snake_head_x)) % (w / cell_size)) } // left
      if (snake_head_x >= Math.round(w / cell_size)) { snake_head_x = Math.abs((w + Math.abs(snake_head_x)) % (w / cell_size)) } // right
      if (snake_head_y <= -1) { snake_head_y = Math.abs((h - Math.abs(snake_head_y)) % (h / cell_size)) } // up
      if (snake_head_y >= Math.round(h / cell_size)) { snake_head_y = Math.abs((h + Math.abs(snake_head_y)) % (h / cell_size)) } // down
    }
    ...
    if (snake_head_x == food.x && snake_head_y == food.y) {
      if (sounds) { snd.play() }
      tail = {x: snake_head_x, y: snake_head_y}
      createFood()
      score += difficulty
    } else {
